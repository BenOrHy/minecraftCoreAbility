        ...
        if (offhandItem.getType() == Material.LODESTONE || (offhandItem.getType() == Material.IRON_INGOT && offhandItem.getAmount() > 18)) {
            ...
            if (!hasAlive) {
                ...
                for (int i = 0; i < golemCount; i++) {
                    ...
                    world.spawnParticle(Particle.EXPLOSION_EMITTER, spawnLoc, 1);
                    world.playSound(spawnLoc, Sound.BLOCK_ANVIL_LAND, 1.5f, 0.6f);

                    Particle.DustOptions summonDust = new Particle.DustOptions(Color.fromRGB(220, 220, 220), 2.0f);
                    for(int j = 0; j < 360; j+=20) {
                        double r = 1.5;
                        double cx = Math.cos(Math.toRadians(j)) * r;
                        double cz = Math.sin(Math.toRadians(j)) * r;
                        world.spawnParticle(Particle.ELECTRIC_SPARK, spawnLoc.clone().add(cx, 0.5, cz), 2, 0, 0.3, 0, 0.1);
                        world.spawnParticle(Particle.DUST, spawnLoc.clone().add(cx, 0.2, cz), 1, 0, 0.5, 0, 0, summonDust);
                    }
                    world.spawnParticle(Particle.BLOCK, spawnLoc.clone().add(0, 1, 0), 100, 0.7, 1.5, 0.7, iron);
                                ...
                                new BukkitRunnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (dist < speed * 1.2) {
                                            ...
                                            double swirlRadius = 1.4;
                                            double angle = moveTicks * 0.8;

                                            Vector offset1 = p1.clone().multiply(swirlRadius).rotateAroundAxis(axis, angle);
                                            Vector offset2 = p1.clone().multiply(swirlRadius).rotateAroundAxis(axis, angle + Math.PI);

                                            Particle.DustOptions swirlDust = new Particle.DustOptions(Color.fromRGB(240, 240, 240), 1.7f);
                                            Location trailCenter = next.clone().add(0, golem.getHeight() / 2.0, 0);

                                            world.spawnParticle(Particle.DUST, trailCenter.clone().add(offset1), 1, 0, 0, 0, 0, swirlDust);
                                            world.spawnParticle(Particle.DUST, trailCenter.clone().add(offset2), 1, 0, 0, 0, 0, swirlDust);

                                            world.spawnParticle(Particle.CRIT, trailCenter, 5, 0.4, 0.4, 0.4, 0.05);
