...
import org.core.coreSystem.cores.VOL2.Stroke.Skill.F;
import org.core.coreSystem.cores.VOL2.Stroke.Skill.Q;
import org.core.coreSystem.cores.VOL2.Stroke.Skill.R;

import java.util.UUID;

public class strCore extends absCore {
    ...
    public strCore(Core plugin, coreConfig tag, Stroke config, Cool cool) {
        super(tag, cool);
        ...
        plugin.getLogger().info("Stroke downloaded...");
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        Player player = event.getPlayer();
        if (!contains(player) || !hasProperItems(player)) return;

        if (event.getAction().name().contains("RIGHT")) {
            config.rLastClickTime.put(player.getUniqueId(), System.currentTimeMillis());
        }
    }
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (config.qStep.containsKey(uuid)) {

            if (config.qInputDelay.getOrDefault(uuid, 0L) > System.currentTimeMillis()) return;
            if (config.qIsSampling.getOrDefault(uuid, false)) return;

            Vector from = event.getFrom().toVector();
            Vector to = event.getTo().toVector();
            Vector movement = to.subtract(from).setY(0);

            if (movement.lengthSquared() > 0.0005) {
                config.qIsSampling.put(uuid, true);
                ((Q) Qskill).startMovementSampling(player, event.getFrom());
            }
        }
    }
    ...
    private void applyAdditionalHealth(Player player, boolean healFull) {
        long addHP = player.getPersistentDataContainer().getOrDefault(
                new NamespacedKey(plugin, "R"), PersistentDataType.LONG, 0L) * 2
    ...
    private boolean hasProperItems(Player player) {
        ...
        return main.getType() == Material.STICK && off.getType() == Material.AIR;
    }

    @Override
    protected boolean isDropRequired(Player player, ItemStack droppedItem){
        ItemStack off = player.getInventory().getItemInOffHand();
        return droppedItem.getType() == Material.STICK && off.getType() == Material.AIR;
    }

    private boolean canUseRSkill(Player player) {
        return !config.rIsCharging.getOrDefault(player.getUniqueId(), false);
    }

    private boolean canUseQSkill(Player player) {
        return !config.qStep.containsKey(player.getUniqueId());
    }
